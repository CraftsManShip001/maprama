...
| --- | --- |
| 도보, 멈춤 | `idle` |
| 도보 | `walk` → `run` → `idle`. 자연스러운 걷기 속도(4.8 km/h)의 1.6배를 넘으면 `run` → `walk` → `idle`. 재생 속도는 캐릭터가 지도 위에서 실제로 지나가는 거리(초당 미터)를 캐릭터 크기로 나눈 값에 맞춥니다. 크기 1인 캐릭터가 4.8 km/h로 움직일 때 1배이고, 최소 0.5배 · 최대 2.2배입니다. 월드의 `unitMeters`와 무관하므로 실제 속도로 이동해도(`timeScale` 1) 발이 미끄러지지 않고, `timeScale` 20이면 지도 위를 20배 빠르게 지나가므로 `run` 클립이 최대 재생 속도로 나옵니다 |
