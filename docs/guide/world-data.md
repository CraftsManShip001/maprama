...
WorldData 월드는 `buildings[]`에 적힌 건물만 그립니다. `plaza`가 있으면 광장
바닥과 벤치를 깔고 기본 카메라를 그쪽으로 맞출 뿐, 그 위에 건물을 세우지
않습니다.

::: warning 동작 변경 (0.1.0)
예전 엔진은 `plaza` 주변에 건물 윤곽이 없으면 그 자리에 8유닛짜리 유리 타워
(`landmark`)를 **합성해서** 그렸습니다. 실제 지도 데이터에 없는 건물이 서 있는
셈이라 제거했습니다. 건물 개수(`buildings.length`)에도 잡히지 않아 알아채기
어려웠고, 이 타워는 회전 애니메이션까지 있어서 온디맨드 렌더링이 멈추지
못하게 만들었습니다.

그 장식에 기대고 있었다면 `buildings[]`에 실제 건물을 넣거나, 직접 만든
오버레이·캐릭터·모델을 광장 좌표에 올려 주세요. `layout: 'town'` /
`layout: 'grid'` 같은 **절차적 월드**는 예전처럼 자기 랜드마크를 그대로
가지고 있습니다 — 애초에 실제 지형이 아니니까요.
:::
