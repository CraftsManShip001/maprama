...
| v2 native engine (MapLibre Native, Fabric/JSI) | `@maprama/engine-native` | **In development, not part of 0.1.0.** M1 and M2a are done on iOS and Android on the official MapLibre SDKs (`engine="native"`: map, camera, gestures, `camera:change`, `project`/`unproject`, 3D buildings in theme colours with time of day, `setBuildingStyle`, map and building presses, map UI, overlay anchors). Labels and procedural worlds (M2b), roofs and facades (M2c), and characters, travel, drops and geofences (M3) follow. |
