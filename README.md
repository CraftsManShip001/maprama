...
| v2 native engine (MapLibre Native, Fabric/JSI) | `@maprama/engine-native` | **Beta candidate, not part of 0.1.0.** M1–M4 are done on iOS and Android on the official MapLibre SDKs (`engine="native"`): map, camera, gestures, `camera:change`, `project`/`unproject`, 3D buildings in theme colours with time of day, `setBuildingStyle`, presses, map UI, overlay anchors (M1/M2a), procedural worlds (M2b, generated by the C++ core: same seed, same world as the web engine), roofs / facades / outlines / captured flag in a custom render layer (M2c), characters, location sources, travel, drops and geofences (M3a) with glTF characters, vehicles and 3D drop items (M3b), and the zoom-out game view with the performance and parity pass (M4). **Labels (M2b) are the remaining gap**; see `packages/engine-native/DESIGN.md` §11. |
