...
/** Raw API key format: `dio_` + 43 base64url characters (32 random bytes). */
export const API_KEY_PATTERN = /^dio_[A-Za-z0-9_-]{43}$/;
...
export async function generateApiKey(crypto: WebCrypto): Promise<{ key: string; keyHash: string }> {
  const key = `dio_${randomToken(crypto, 32)}`;
  return { key, keyHash: await sha256Hex(crypto, key) };
}

/**
 * Per-app receipt secret: `hex(HMAC-SHA256(masterSecret, "diorama-receipt:v1:" + appId))`.
...
export async function deriveReceiptSecret(masterSecret: string, appId: string): Promise<string> {
  return toHex(await hmacSha256(masterSecret, `diorama-receipt:v1:${appId}`));
}
