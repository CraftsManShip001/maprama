...
import type { CameraState } from '@diorama/protocol';
 ...
 *
 * @param map the map ref or API; inside `DioramaMap` pass `null` to use the enclosing map.
 */
export function useCameraState(map: DioramaMapRefLike, options: UseCameraStateOptions = {}): CameraState | null {
  ...
  const [state, setState] = useState<{ api: DioramaMapRef; value: CameraState } | null>(null);
