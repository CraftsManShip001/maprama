...
void LabelSystem::layoutHolo(const LabelLayoutInput& in, const MapProjector& projector,
  ...
  for (std::size_t i = 0; i < entries_.size(); ++i) {
    ...
    const MapProjector::Point g = projector.project(e.lngLat, kLabelGroundY * in.unitMeters);
    const MapProjector::Point t = projector.project(e.lngLat, (kLabelGroundY + holoHeight(e.kind)) * in.unitMeters);
