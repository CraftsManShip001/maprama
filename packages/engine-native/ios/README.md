...
| `AppleMapAdapter` (in `MapramaNativeView.mm`) | C++ `maprama::MapAdapter` | `styleJSON`, zoom/pitch limits, `setCamera:` (altitude from `MLNAltitudeForZoomLevel`), `convertCoordinate:` / `convertPoint:` for project/unproject, `NSURLSession` for `world.kind = "url"` and glTF models (`fetchBinary`, M3b), `dispatch_after` frames, coalesced main-thread hops for model frames (M3b); ImageIO decodes glTF textures (`EngineConfig::decodeImage`). Always hops to the main queue asynchronously and replies through the engine's `on*` methods. |
| `MapramaBuildingLayer` (`MapramaBuildingLayer.h/.mm`, M2c) | `MLNCustomStyleLayer` subclass (Metal) | Draws the core's `BuildingLayerData` (roofs, facade windows / details, outlines, captured flag) with MapLibre's `renderEncoder` and `nearClippedProjectionMatrix`, depth-shared with the `buildings` fill-extrusions; inserted below `buildings` after every style load. Since M3b also the core's `ModelLayerFrame` (`setModelFrame:`): a second Metal pipeline pair skins the characters, vehicles and drop items on the GPU (palette + instance buffers in a ring of three, meshes / textures cached by `ModelMesh::id`), in the same pass and depth range. Logs `maprama-frame-stats` (frame interval, frame GPU time, layer encode time, model count / draws / encode time) to `os_log` category `building-layer`. |
