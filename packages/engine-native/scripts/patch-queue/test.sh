...
sed 's/^line 2$/diorama line 2/' "$work/core.txt" > "$work/core.tmp" && mv "$work/core.tmp" "$work/core.txt"
git -C "$work" commit -q -am "[diorama] patch core line 2"
echo "diorama layer" > "$work/diorama.txt"
git -C "$work" add diorama.txt && git -C "$work" commit -q -m "[diorama] add diorama layer"
...
expect_eq "patch files are numbered" "$(cd "$queue" && ls *.patch | head -n 1)" "0001-diorama-patch-core-line-2.patch"
expect_eq "patches use zero commit ids" "$(head -n 1 "$queue/0001-diorama-patch-core-line-2.patch" | cut -d' ' -f2)" \
...
expect_eq "apply creates the patched branch" "$(git -C "$clone" rev-parse --abbrev-ref HEAD)" "diorama/patched"
expect_eq "apply adds both commits" "$(git -C "$clone" rev-list --count v1..HEAD)" "2"
expect_eq "patched content present" "$(sed -n 2p "$clone/core.txt")" "diorama line 2"
expect_eq "added file present" "$(cat "$clone/diorama.txt")" "diorama layer"
...
expect_eq "patch still applied after rebase" "$(sed -n 2p "$clone/core.txt")" "diorama line 2"
