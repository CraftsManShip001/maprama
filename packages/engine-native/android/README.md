...
| `maprama_jni.cpp` (`src/main/cpp`) | C++ | `JniMapAdapter` (`MapAdapter` → `MapramaMapHost`), `JniMessageSink` (events → `MapramaEngineModule.dispatchEvent`, logs → logcat tag `MapramaEngine`), engine handles, UTF-16 ↔ UTF-8 conversion; M3b: model frames (`modelLayerChanged` → coalesced `triggerRepaint`), `fetchBinary` replies, glTF texture decoding through `MapramaJni.decodeImage` (BitmapFactory) on worker threads, which are detached from the VM when they exit. |
| `maprama_building_layer.cpp` (`src/main/cpp`, M2c) | C++ `mln::style::CustomLayerHost` (GL ES 3) | Draws the core's `BuildingLayerData` (roofs, facade windows / details, outlines, captured flag) in MapLibre's render pass with its `nearClippedProjectionMatrix` and the extrusions' depth range (read back from `GL_DEPTH_RANGE`). Registered from Kotlin as `CustomLayer("maprama-buildings-3d", MapramaJni.createBuildingLayerHost(handle))` below `buildings`; shares `BuildingLayerState` with `JniMapAdapter`. Since M3b it also draws the core's `ModelLayerFrame` with a GL ES 3 skinning program (palette in a std140 uniform block, divisor-1 instance attributes, meshes / textures cached by `ModelMesh::id`) in the same depth range. Logs `maprama-frame-stats` (incl. model count / draws / render time) to logcat tag `MapramaBuildingLayer`. |
