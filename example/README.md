...
| 9 | `markers.tsx` | `MarkerLayer`: ~40 server-provided POI pins in two faction colours (custom SVG icons for the partner and station pins), priority collision shared with the map labels, partner pins that are never hidden, a selected pin scaled by `selectedScale`, a "server tick" button that changes only colours and the selection (no marker rebuild), and a tap that opens a sheet at the reported screen point while the map's `onPress` / `onBuildingPress` stay silent. |
| 10 | `camera.tsx` | `minDistanceMeters` / `maxDistanceMeters` in real metres, a 3,330 m wide view, and `fitBounds` over 18 pins on a 1.6 km ring. |
| 11 | `sheet.tsx` | The bottom-sheet screen: an opaque sheet over half the display, a `ui.contentInset` toggle, and a live `camera:idle` readout (centre, ground `bounds`, `radiusMeters`, `reason`). With the inset on, the engine-drawn `© OpenStreetMap` attribution, the scale bar and the zoom buttons move above the sheet and the camera centres in the visible half. |
| 12 | `native.tsx` | `engine="native"` M1 + M2a: 3D MapLibre buildings in theme colours, themes and time of day, a captured building, map / building presses, a `MapOverlay` card, map UI, camera presets and a project/unproject round trip. |
| 13 | `native-game.tsx` | `engine="native"` M3a game systems as style layers: a walker on the `simulated` location source, `travel` to near / far presets at ×20 with a route line and live ETA, two drops placed halfway along the `route` request's paths and collected on the way, a geofence with enter / exit events, the location puck and camera follow. |
